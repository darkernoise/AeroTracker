...
class AT_DataProcServerWorker(AT_ProcessBase):
    ...
    _local_address = ''
    _socket_srvr = None
    _trilat_worker = AT_TrilaterationWorker #type AT_TrilaterationWorker
    _worker_num = 0
    ...
    def _init_trilateration(self):
        ...
        self._trilat_worker = AT_TrilaterationWorker(
            dir_store=self._dir_store,
            pipe_pool=self._pipe_pool, 
            params=self.params)
    ...
    def __init__(self, dir_store, params, worker_num):
        ...
        self._pipe_pool = self._get_new_pipe_pool()
        return
    
    def _get_new_pipe_pool(self)->AT_PipePool:
#         rval = AT_PipePool(params=self.params, num_pipes=self.params.NUM_SENSOR_CLUSTERS)
        rval = AT_PipePool(params=self.params, num_pipes=1)
        return rval
