...
#     _manager_port = 99
    ...
    def get_data_port_for_target(self, target_id:str, cluster_id:str)->AT_DataProcAddress:
        '''
        Requests assignment of a data processing server for a given target id and cluster id.
        '''
        self._lock.acquire()
        ...
        for assgn in self._data_proc_srvr_assignments:
            if ((assgn[0] == target_id) and (assgn[1] == cluster_id)):
                srvr = assgn[2] #type AT_NetworkAddress
                self._lock.release()
                return srvr
        ...
        if (srvr != None):
            self._data_proc_srvr_assignments.append([target_id, cluster_id, srvr])
    ...
    def get_unused_data_proc_srvr(self)->AT_DataProcAddress:
        '''
        Finds an unassigned data processing server.
        '''        
        for srvr in self._data_proc_srvrs:
            found = False
            for assgn in self._data_proc_srvr_assignments:
                if ((srvr.address == assgn[2].address) and 
                    (srvr.port == assgn[2].port)):
                    found = True
                    exit
            if (not found):
                return srvr
